...
signatures <- as.list(read.table("/data-cbl/mquatre/p2_scrnaseq_renal_landscape/data/external/220602_p2_signatures_scrnaseq.csv", header = T, sep = ";", fill = TRUE))
...
set.seed(1)
# Setting the signature of cell types as a list
signatures <- as.list(read.table("/data-cbl/mquatre/p2_scrnaseq_renal_landscape/data/external/220602_p2_signatures_snrnaseq.csv", header = T, sep = ";", fill = TRUE))
signatures <- lapply(signatures, function(x){ x[!is.na(x) & x != ""]}) # removing empty elements from signatures
